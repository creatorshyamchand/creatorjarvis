...
def process_user_query(user_query_text: str):
    """
    Process user query with JARVISAssistant and return response
    """
...
if __name__ == '__main__':
    ...
    try:
        # Try launching in Chrome
        eel.start('index.html', mode='chrome', size=(990, 540), block=True, host='localhost', port=8000)
    except Exception as e:
        print(f"Could not start Eel using Chrome: {e}")
        print("Trying to start in default browser...")
        try:
            eel.start('index.html', mode='default', size=(990, 540), block=True, host='localhost', port=8000)
        except Exception as e2:
            print(f"Still failed to launch UI: {e2}")
    finally:
